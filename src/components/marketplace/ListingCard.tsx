    ...
    <Link href={`/marketplace/${item.id}`} className={`group block min-w-0 ${className}`}>
      <article className="surface-card h-full overflow-hidden rounded-lg">
        <div className="relative aspect-[5/4] overflow-hidden bg-[var(--sand)]">
          ...
          <span className={`absolute left-1.5 top-1.5 inline-flex items-center gap-1 rounded-full px-1.5 py-1 text-[0.57rem] font-bold md:left-3 md:top-3 md:px-2.5 md:text-xs ${intent.className}`}>
            <IntentIcon size={10} className="md:h-3.5 md:w-3.5" aria-hidden="true" />
            {intent.label}
          </span>
        </div>

        <div className="p-2 md:p-4">
          <h3 className="line-clamp-1 text-[0.74rem] font-bold leading-tight text-[var(--foreground)] md:text-base">
            {item.title}
          </h3>
          <p className="mt-1 truncate text-[0.78rem] font-bold text-[var(--brand)] md:text-lg">
            {item.price ? formatCurrency(Number(item.price)) : 'Free'}
          </p>
          <div className="mt-1.5 flex min-w-0 items-center gap-1 text-[0.58rem] font-semibold text-[var(--muted-foreground)] md:text-xs">
            <MapPin size={10} className="shrink-0 md:h-3 md:w-3" aria-hidden="true" />
            <span className="truncate">{item.city || 'Location not set'}</span>
          </div>
          <p className="mt-1.5 truncate text-[0.55rem] font-medium text-[var(--muted-foreground)] md:text-[0.7rem]">
