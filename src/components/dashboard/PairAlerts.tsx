  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-end justify-center bg-black/25 sm:items-center sm:p-5" role="presentation">
      <form
        ...
        className="max-h-[92dvh] w-full overflow-y-auto rounded-t-card border border-[var(--border)]/55 bg-white px-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] pt-4 sm:max-w-xl sm:rounded-card sm:p-6"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <label className="space-y-1.5 sm:col-span-2">
            ...
            <Input value={form.query} onChange={(event) => setValue('query', event.target.value)} minLength={2} maxLength={120} required className="h-11 text-base" placeholder="Lid for a 24 cm cooking pot" autoFocus />
          </label>
          <label className="space-y-1.5">
            <span className="text-xs font-bold sm:text-sm">Category</span>
            <Select value={form.category} onChange={(event) => setValue('category', event.target.value)} required className="h-11 px-3">
          ...
          <label className="space-y-1.5">
            ...
            <Select value={form.city} onChange={(event) => setValue('city', event.target.value)} className="h-11 px-3">
          ...
          <label className="space-y-1.5">
            ...
            <Input value={form.brand} onChange={(event) => setValue('brand', event.target.value)} maxLength={80} className="h-11 text-base" />
          </label>
          <label className="space-y-1.5">
            ...
            <Input value={form.model} onChange={(event) => setValue('model', event.target.value)} maxLength={80} className="h-11 text-base" />
          </label>
          <label className="space-y-1.5">
            <span className="text-xs font-bold sm:text-sm">Side or position</span>
            <Select value={form.side} onChange={(event) => setValue('side', event.target.value)} className="h-11 px-3">
          ...
          <label className="space-y-1.5">
            ...
            <Input type="number" min="0" step="0.01" value={form.budget} onChange={(event) => setValue('budget', event.target.value)} className="h-11 text-base" placeholder="NGN" />
          </label>
          <label className="space-y-1.5 sm:col-span-2">
            ...
            <Textarea value={form.description} onChange={(event) => setValue('description', event.target.value)} maxLength={1000} rows={3} className="min-h-[86px] text-base" placeholder="Colour, dimensions, or anything that helps identify the right fit" />
          </label>
        </div>

        <Button type="submit" disabled={busy} className="mt-4 h-11 w-full bg-[var(--brand)] font-bold text-white">
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-3 md:space-y-6">
      <header className="flex items-center justify-between gap-3 border-b border-[var(--border)]/70 pb-3 md:pb-5">
        ...
        <Button type="button" onClick={() => setShowForm(true)} className="h-10 shrink-0 bg-[var(--brand)] px-3 text-xs font-bold text-white md:px-5 md:text-sm">
          <Plus size={16} />
          New alert
        </Button>
      </header>
                      ...
                      {alert.matches.slice(0, 4).map((match) => (
                        <Link key={match.id} href={`/marketplace/${match.listing.slug || match.listing.id}`} className="group flex min-w-0 items-center gap-2 rounded-control border border-[var(--line-soft)] p-2 transition-colors hover:border-[var(--brand)]/40 hover:text-[var(--brand)]">
                          ...
                          <span className="min-w-0 flex-1">
                            ...
                            <span className="mt-0.5 block text-xs font-semibold text-[var(--brand)]">{Math.round(match.score * 100)}% match</span>
                          </span>
